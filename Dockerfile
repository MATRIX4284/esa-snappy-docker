...
COPY snap_update.sh .
RUN chmod 777 snap_update.sh
# update SNAP from Web, requires font
#RUN /usr/local/snap/bin/snap --nosplash --nogui --modules --update-all 2>&1 | while read -r line; do ( echo "$line" [ "$line" = #"updates=0" ] && sleep 2 && pkill -TERM -f "snap/jre/bin/java" && break ) done
RUN ./snap_update.sh
